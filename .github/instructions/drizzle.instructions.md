...
- Add a TSDoc/JSDoc comment to every exported function in `db/` and `src/lib/`. The comment must state the function's purpose, document every parameter (including the injectable `db` argument), and describe the return value. Keep the comment focused on intent and the contract; do not paraphrase the implementation.
- Update or remove comments when the implementation changes. Treat stale documentation as a correctness defect.
